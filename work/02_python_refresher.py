...
average = sum_prices * 1.0 / len(data)
print average  

# step 4 
def away_team_finder(): 
    import csv
    r = requests.get('https://raw.githubusercontent.com/fivethirtyeight/data/master/nfl-ticket-prices/2014-average-ticket-price.csv')
    data = [row for row in csv.reader(r.iter_lines())]
    data = data[1:97]
...
def home_team_finder():
    ...
    home_teams = []
    for row in data: 
        home_team = row[0].split(" at ", 1)[1]
        home_team = home_team.split(" Tickets ", 1)[0]
...
# step 8
sum_prices = sum(ravens_home_prices)
average_home = sum_prices * 1.0 / len(ravens_home_prices)
print average_home

sum_prices = sum(ravens_away_prices)
average_away = sum_prices * 1.0 / len(ravens_away_prices)
print average_away
